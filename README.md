...
Connect the database API using the `db_api_connect()` function

### A specific dataset

Connect the database API with the default parameters. The default dataset is `dataset_adisser17`
...
Collect all data from API-available datasets by using the option: `all_datasets = TRUE`. By default, all API available By default, all API-available datasets are listed in the CHIPS table hosted on GitHub: [urls_data.tsv](https://github.com/iramat/iramat-dev/blob/main/dbs/chips/urls_data.tsv)
