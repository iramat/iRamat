...
chrono <- function(d,
  ...
  if (seriated) {
    df_sites <- df_sites %>%
      dplyr::mutate(context_name = forcats::fct_reorder(
        factor(context_name),
    ...
    df_sites <- df_sites %>%
      dplyr::mutate(context_name = factor(context_name))
  }
  ...
  unique_sitename <- length(unique(df_sites$context_name))
  ...
  p_sites <- ggplot2::ggplot(df_sites) +
    ggplot2::geom_segment(
      ggplot2::aes(x = start_num, xend = end_num,
                   y = context_name, yend = context_name, linetype = type),
      linewidth = 1, color = "gray30", na.rm = TRUE
    ) +
    ggplot2::geom_point(
      ggplot2::aes(x = start_num, y = context_name, shape = type),
      size = 3, color = "black", na.rm = TRUE
    ) +
    ggplot2::geom_point(
      ggplot2::aes(x = end_num, y = context_name, shape = type),
      size = 3, color = "black", na.rm = TRUE
    ) +
    ...
    ggplot2::theme(
      axis.text.y = ggplot2::element_text(
        colour = ifelse(levels(df_sites$context_name) %in% df_sites$context_name[df_sites$dated],
                        "black", "grey50")
      )
    )
